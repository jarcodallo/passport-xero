...
PatchedOauth.prototype._createClient= function( port, hostname, method, path, headers, sslEnabled ) {
  var options = {
    ...
    method: method,
    headers: headers,
    key: fs.readFileSync(process.cwd() + '/server/dataSources/Infusionsoft/certificates/entrust-private-nopass.pem', 'utf8'),
    cert: fs.readFileSync(process.cwd() + '/server/dataSources/Infusionsoft/certificates/entrust-cert2.pem', 'utf8')
  };
  var httpModel;
  if( sslEnabled ) {
    httpModel= https;
